...
def export_to_csv(attendance_records, course=None, start_date=None, end_date=None):
    """
    ...
    # Write header row
    writer.writerow([
        'Attendance Management System - Attendance Report',
        f'Course: {course.name if course else "All Courses"}',
    ...
    writer.writerow([
        'Date', 'Student', 'Course', 'Section', 'Status', 'Marked By', 'Time'
    ])
    ...
    for record in attendance_records:
        writer.writerow([
            ...
            record.user.name,
            f"{record.course.name} ({record.course.code})",
            'N/A',  # Section relationship removed
            record.status,
            record.marked_by_user.name,
